...
type
  TFormCalculadora = class(TForm)
    ...
    procedure Image_multiplicaçãoClick(Sender: TObject);
    procedure Image_virgulaClick(Sender: TObject);
    procedure Image_apagarClick(Sender: TObject);

  private
    { Private declarations }
    procedure Escreve_Visor(valor: string);  //Mostra pro usuário qual número foi selecionado
    procedure Escreve_Informacao(valor: string); //Mostra qual a operação está sendo feita
    procedure Limpa_Visor(valor: string);  // Limpa as informações somente do visor
    procedure Escreve_valor1(valor: string;operacao: string); //Mostra na tela o primeiro número digitado
    procedure Escreve_valor2(valor: string;resultado: string); //Mostra na tela o primeiro número digitado

  public
    { Public declarations }
   procedure Limpa_Tudo(valor: string);  // Limpa todas as informações
  end;

var  //Variáveis Necessárias
...
// Procedure para escrever no painel de informação qual é a operação selecionada.
procedure TFormCalculadora.Escreve_Informacao(valor: string);
begin
    if (valor = '/') then
    Label_informacao.Text := 'Operação: Divisão (/)'
  else if (valor = '*') then
    Label_informacao.Text := 'Operação: Multiplicação (*)'
  else if (valor = '+') then
    Label_informacao.Text := 'Operação: Soma (+)'
  else if (valor = '-') then
    Label_informacao.Text := 'Operação: Subtração (-)'
  else if (valor = '%') then
    Label_informacao.Text := 'Operação: Porcentagem (%)'
...
// Procedure para escrever os valores digitados no visor de números.
...
begin
  //Verifica se foi clicado o botão tento já informações na tela
  ...
  if Label_valor.Text = '' then      //Se não tiver valor de operações
...
begin
  ...
  begin
    ...
    //verificar se a operação é de subtração
    if operacao = '-' then
    begin
    ...
    //verificar se a operação é de adição
    if operacao = '+' then
      begin
      resultado := valor1 + valor2;
      Escreve_valor2(valor2.ToString(),resultado.ToString());
    ...
    if operacao = '/' then
      begin
      resultado := valor1 / valor2;
      if (resultado.ToString() = 'INF') then    //Valida Divisão por Zero
    ...
    if operacao = '*' then
     ...
     //Verifica se a operação é Porcentagem
     if operacao = '%' then
     begin
...
procedure TFormCalculadora.Image_multiplicaçãoClick(Sender: TObject);
begin
  //Verifica se foi clicado o botão tento já informações na tela
  // Ou seja, segue com a conta
  if (Label_valor.Text <> '') and (Label_informacao.Text <> '') and
